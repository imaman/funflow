...
var Fork = Composite.extend({
  ...
  setFuncName: function(arg) { this.funcName_ = arg; return this },
  funcName: function() { return this.funcName_ },
...
function timer(name, frequencyIsMillis, timeoutInMillis, err, value) {
  if (name !== undefined && !util.isString(name)) {
    var args = [undefined].concat(u_.toArray(arguments));
    return timer.apply(null, args);
  }
  var fireCustomResult = (arguments.length >= 4);
  timeoutInMillis = timeoutInMillis || 60*1000;
  var active = false;
  return single(name, function() {
...
function single(name, f, inArity) {
  if (util.isFunction(name)) {
    return single(undefined, name, f);
  }
  if (inArity === undefined)
    inArity = f.length + 1;
  return Computation.new_(wrap(f), inArity, name || f.name).setOutArity(1);
}
...
function treeFromDsl(dsl, prefix) {
  ...
  function translate(dsl) {
    ...
    if (dsl.up && dsl.up() === ForkOperator) {
      ...
      })).setMerge(function(inputs, next, dispatching) {
        return dsl.merge.call(dispatching.execution.context, inputs, next);
      }).setFuncName(dsl.merge.name);
    }
