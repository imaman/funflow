...
function run(dsl, expected) {
  ...
  function pump() {
    while (gameOn && completed.length > 0) {
      ...
      outgoingArgs.push(function() {
        var output = u_.toArray(arguments);
        t.output = output;
        console.log(t + ' => ', JSON.stringify(t.output));
        completed.push(t);
      });
...
run(['A', function(v, next) { process.nextTick(function() {next(null, v + 'B') })}], 'AB_');
