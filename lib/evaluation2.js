...
function normalize(v) {
  ...
  console.log('v=' + v + ' #sources=' + sources.length + ' .payload? ' + (v.payload !== null));
  ...
  if (isMerge) {
    v.payload = function() {
       ...
       v.incoming().forEach(function(edge) {
         inputs[edge.slot] = edge.from.output.slice(1);
       });
       next(null, inputs);
    }
    return;
    ...
    try {
      while (completed.length > 0) {
        ...
        ts.filter(isReady).forEach(function(t) {
          ...
          console.log('evaling ' + t + ' with ' + JSON.stringify(outgoingArgs));
