...
var Compiler = Top.extend({
  compile: function(dsl) {
    ...
    return Flow.new_(node, v0);
  }
}, function(defs, options) { return { options: options } });

function compile(dsl, options) {
  options = extend({ branchOp: 'SINGLE' }, options);
  var compiler = Compiler.new_(options);
