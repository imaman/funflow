...
var Execution = Top.extend({
  ...
  continueFrom: function(state) {
    ...
    function dispatchNextVertex(t, output) {
      ...
      var nextOfT = next.bind(null, self.stateFromVertex(t));
  ...
  return { buf_: [], callback: callback, root_: root, statesToProcess_: [],
...
function payloadWithExecution(v) {
  ...
  if (execution.stateFromVertex(v).completed) {
    return;
  }
  var next = args.pop();
  ...
  var inputs = {};
  var err;
  var completedEdges = v.incoming().filter(function(edge) {
    return execution.stateFromVertex(edge.from).completed
  });
  completedEdges.forEach(function(edge) {
    var s = edge.from;
    var outputOfS = execution.stateFromVertex(s).output;
