...
var Execution = Top.extend({
  ...
  continueFrom: function(step) {
    ...
    function computeOugoingArgs(t, output) {
      var outgoingArgs = [].concat(output.asArray());
      var nextOfT = next.bind(null, self.stepFromVertex(t));
      outgoingArgs.push(nextOfT);
      outgoingArgs.push(self);
      return { args: outgoingArgs, next: nextOfT };
    }

    function dispatchNextVertex(toStep, fromStep) {
      var outgoingArgs = computeOugoingArgs(toStep.vertex, fromStep.output);
      try {
        toStep.vertex.payloadWithExecution.apply(null, outgoingArgs.args);
      } catch (err) {
        outgoingArgs.next(err);
      }
    }
    ...
    function processCompleted(step) {
      ...
      step.nexts.forEach(function(t) {
        dispatchNextVertex(self.stepFromVertex(t), step);
      });
    }
  },
  toString: function() {
