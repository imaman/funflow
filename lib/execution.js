    ...
    function pump() {
      ...
      try {
        while (execution.completed_.length > 0) {
          ...
          var outputsOfCurr = execution.inputs_;
          if (curr !== execution.v0_)
            outputsOfCurr = execution.get(curr, 'output');
          console.log('curr=' + curr + ', outputsOfCurr=' + JSON.stringify(outputsOfCurr));

          var ts = curr.targets();
          if (ts.length === 0) {
            execution.isDone_ = true;
            return execution.callback.apply(null, outputsOfCurr);
          }
          ts.forEach(function(t) {
            var outgoingArgs = [].concat(outputsOfCurr);
...
}, function(defs, callback, root, v0, inputs) {
  return { buf_: [], callback: callback, root_: root, v0_: v0, completed_: [], pumpOn_: false, isDone_: false, inputs_: inputs };
});
