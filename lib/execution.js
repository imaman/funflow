...
var Step = Top.extend({
  ...
  computeDispatchingTo: function(toStep, execution, next) {
    var outgoingArgs = [].concat(this.output.asArray());
    var nextWithStep = next.bind(null, toStep);
    outgoingArgs.push(nextWithStep);
    return { args: outgoingArgs, next: nextWithStep, execution: execution, toStep: toStep };
  }
...
var Execution = Top.extend({
  ...
  continueFrom: function(step) {
    ...
    function dispatch(fromStep, toStep) {
      var dispatching = fromStep.computeDispatchingTo(toStep, self, next);
      try {
...
function createDispatchable(v, isMerge) {
  if (isMerge)
    return mergingDispatchable.bind(null, v);
  return function(dispatching) {
    v.payload.apply(null, dispatching.args);
  }
}

function mergingDispatchable(v, dispatching) {
