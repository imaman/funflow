...
var Step = Top.extend({
  ...
  outputAsArray: function() {
    return this.output.asArray(this.vertex && this.vertex.outArity)
  }
  ...
  continueFrom: function(step) {
    ...
    function processCompleted(step) {
      ...
      step.nexts.forEach(function(t) {
        dispatchNextVertex(t, step.output);
      });
    }
  },
  toString: function() {
...
function payloadWithExecution(v) {
  ...
  var completedEdges = v.incoming().filter(function(edge) {
    return execution.stepFromVertex(edge.from).completed
  });
  completedEdges.forEach(function(edge) {
    var s = edge.from;
    var step = execution.stepFromVertex(s);
    var outputOfS = step.output;
    ...
    inputs[edge.slot] = step.outputAsArray();
  });
