...
var Step = Top.extend({
  ...
  computeDispatchingTo: function(toStep, execution, next) {
    ...
    return { args: outgoingArgs, next: nextWithStep };
  }
...
var Execution = Top.extend({
  ...
  continueFrom: function(step) {
    ...
    function dispatch(fromStep, toStep) {
      ...
      var args = dispatching.args.slice(0);
      args.unshift(dispatching);

      try {
        toStep.vertex.payloadWithExecution.apply(null, args);
      } catch (err) {
...
function createPayloadWithExecution(v, isMerge) {
  ...
  return function() {
    var args = u_.toArray(arguments).slice(1);
    args.pop();
