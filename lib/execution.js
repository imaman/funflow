...
var Execution = Top.extend({
  ...
  outputOf: function(name) {
    var steps = this.buf_.filter(function(curr) {
      return curr.vertex.dsl.name() === name;
    });
    if (steps.length === 1) return steps[0].output.asArray();
    throw new Error('Vertex ' + id + ' was not (uniquely) found');
  },
  ...
  traverse(realV0);
  var buf = [];
  u_.range(max + 1).forEach(function(key) {
    var v = vertexById[key];
    buf.push(Step.new_(v));
  });
  return { buf_: buf, callback: callback, root_: root, stepsToProcess_: [],
