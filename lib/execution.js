...
var Execution = Top.extend({
  ...
  inspect: function() {
    ...
    lines = lines.concat(this.buf_.map(function(curr, index) {
      return '  - ' + index + ' => ' + JSON.stringify(curr.output)
    }));
    return lines.join('\n');
  }
}, function(defs, callback, root, inputs, realV0) {
  return { buf_: [], callback: callback, root_: root, completed_: [], pumpOn_: false, isDone_: false, inputs_: inputs, realV0_: realV0, runCount_: 0 }
});
