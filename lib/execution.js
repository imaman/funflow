...
function payloadWithExecution(v) {
  ...
  var steps = v.incoming().map(function(edge) {
    return execution.stepFromVertex(edge.from)
  });
  var edgesAndSteps = u_.zip(v.incoming(), steps);
  var completed = edgesAndSteps.filter(function(current) {
  ...
  completed.forEach(function(current) {
    ...
    err = err || step.storeAt(inputs, edge.slot);
  });
