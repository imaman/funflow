...
var Execution = Top.extend({
  run: function(e) {
    ...
    this.callback = next;
    var self = this;
    return this.continueFrom({ vertex: null, output: self.inputs_, nexts: [self.realV0_] });
  },
  ids: function() {
  ...
  continueFrom: function(state) {
    ...
    function next(t) {
      ...
      self.continueFrom(computeState(t));
    }
    ...
    function pump() {
      ...
      self.pumpOn_ = true;
      try {
        while (self.completed_.length > 0) {
          var state = self.completed_.pop();
          markCompletionOf(state.vertex);
          processCompletedVertex(state);
        }
      } finally {
        self.pumpOn_ = false;
      }
    ...
    function markCompletionOf(v) { v && self.put(v, 'completed', true) }
    function startOfFlow(v) { return v === null }
    function endOfFlow(v) { return v && v.outgoing().length === 0 }
    function finalize(v) {
      var outputs = self.get(v, 'output').asArray();
      self.isDone_ = true;
      return self.callback.apply(null, outputs);
    }

    function dispatchNextVertex(t, output) {
      var outgoingArgs = [].concat(output);
    ...
    function computeState(v) {
      if (startOfFlow(v)) {
        return { vertex: v, output: self.inputs_, nexts: [self.realV0_] }
      }

      return { vertex: v, output: self.get(v, 'output').asArray(), nexts: v.targets() }
    }

    function processCompletedVertex(state) {
      var v = state.vertex;
      if (endOfFlow(v))
        return finalize(v);
