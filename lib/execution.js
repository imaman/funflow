...
var Execution = Top.extend({
  ...
  continueFrom: function(t) {
    ...
    function pump() {
      ...
      try {
        while (self.completed_.length > 0) {
          ...
          markCompletionOf(curr);
          processCompletedVertex(curr);
        }
      } finally {
        self.pumpOn_ = false;
      }
    ...
    function dispatchNextVertex(t, outputOfCurr) {
    ...
    function computeState(curr) {
      if (startOfFlow(curr)) {
        return { output: self.inputs_, nexts: [self.realV0_] }
      }

      return { output: self.get(curr, 'output').asArray(), nexts: curr.targets() }
    }

    function processCompletedVertex(curr) {
      if (endOfFlow(curr))
