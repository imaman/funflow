...
var Execution = Top.extend({
  ...
  get: function(vertex) {
    return this.buf_[vertex.key];
  },
  put: function(vertex, value) {
  ...
  continueFrom: function(state) {
    ...
    function markCompletionOf(state) { state.completed = true }
    function endOfFlow(state) { return state.isEndOfFlow() }
    ...
    function stateFromVertex(v) {
      ...
      if (res)
        return res;
      res = State.new_(v, null, v.targets());
      self.put(v, res);
      return res;
    }
...
function payloadWithExecution(v) {
  ...
  var completedEdges = v.incoming().filter(function(edge) {
    return execution.get(edge.from).completed
  });
