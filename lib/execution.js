...
var Output = Top.extend({
  ...
  translateError: function(execution) {
    ...
    Object.defineProperty(newErr, 'execution', {
      enumerable: false,
      configurable: false,
      writable: false,
      value: execution
...
var Execution = Top.extend({
  ...
  continueFrom: function(step) {
    ...
    function finalize(step) {
      self.isDone_ = true;
      return self.callback.apply(self.context, step.output.asArray());
    }
  },
  toString: function() {
  ...
  return { buf_: orderedVertices.map(function(v) { return Step.new_(v) }), callback: callback, root_: root, stepsToProcess_: [],
    ...
    runCount_: 0, options_: options || {}, context: {} }
});
...
function createDispatchable(isMerge) {
  ...
  return function(dispatching) {
    dispatching.toStep.vertex.payload.apply(dispatching.execution.context, dispatching.args);
  }
}
...
function mergingDispatchable(dispatching) {
  ...
  vertex.merge(inputs, dispatching.next, dispatching);
}
