...
var Step = Top.extend({
  ...
  computeDispatching_: function(toStep, execution, next) {
    ...
    outgoingArgs = [].concat(this.output.asArray());
    outgoingArgs.push(nextWithStep);

    arity = outgoingArgs.length;
    if (arity != undefined) {
      var temp = u_.range(arity).map(function(index) {
        if (index === arity - 1) return nextWithStep;
        if (index >= dataToSend.length) return undefined;
