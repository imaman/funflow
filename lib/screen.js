...
var Screen = Top.extend({
  ...
  computeCell_: function(es0, c, spacing, widths) {
    var s = ' ';
  ...
  render: function(spacing) {
    ...
    var lines = u_.range(0, max.r + 1).map(function(r) {
      var line = u_.range(0, max.c + 1).map(function(c) {
        var es0 = this.findEntry_(r, c);
        return this.computeCell_(es0, c, spacing, widths);
      }, this);
