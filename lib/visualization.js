...
function showFork(v, screen, options, preOrder) {
  ...
  var func = v.merge();
  if (func) {
    screen.putAt(row + 2, 0, '|');
    screen.putAt(row + 3, 0, 'merge#3');
    screen.putAt(row + 4, 0, '|');
    deltaRow += 3;
  }
...
function showComputation(v, screen, options, preOrder) {
  ...
  function preOrder(v, screen) {
    return v.accept({
      ...
      computation: function(v) {
        return showComputation(v, screen, options, preOrder);
      }
    });
  }
